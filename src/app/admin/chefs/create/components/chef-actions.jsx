...
export default function ChefActions({ chef }) {
  ...
  // Защита от undefined chef
  if (!chef) {
    return (
      <div className="flex flex-wrap gap-3 animate-fade-in">
        <div className="px-6 py-3 rounded-xl bg-gray-300 text-gray-600 font-medium flex items-center">
          <span className="mr-2">
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
          </span>
          Данные повара не загружены
        </div>
      </div>
    )
  }
  ...
  const buttons = [
    ...
    {
      key: 'status',
      label: chef.status === 'active' ? 'Заблокировать' : 'Активировать',
      onClick: handleStatusToggle,
      color: chef.status === 'active' ? 'orange' : 'green',
      icon: (
        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          {chef.status === 'active' ? (
  ...
  const getButtonClasses = (color, isHovered) => {
    ...
    const colorClasses = {
      ...
      green: `bg-gradient-to-r from-green-500 to-emerald-600 hover:from-green-600 hover:to-emerald-700 ${isHovered ? 'shadow-green-200' : ''}`,
      orange: `bg-gradient-to-r from-orange-500 to-amber-600 hover:from-orange-600 hover:to-amber-700 ${isHovered ? 'shadow-orange-200' : ''}`,
